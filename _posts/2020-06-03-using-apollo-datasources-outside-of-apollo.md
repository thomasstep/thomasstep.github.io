...
The reason that the calls fails if the DataSource is not initialized is because there is never any HTTPCache created for the instance of `RESTDataSource`.
...
Either way, this is the problem, solution, and reasoning for using Apollo Server DataSource outside of an Apollo Server.
