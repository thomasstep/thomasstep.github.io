...
API Gateway has a few resources that are the building blocks for an API's structure. An API resource itself (the resource is also called a Rest API or a HTTP API) is an entity that separates a particular API Gateway instance from other APIs and is accessible by its own URL. The API is then made up of various resources which correspond to API paths. Each resource which defines a path has methods attached to it which define which HTTP verbs can be used to interact with the resources held configured to handle the request.
...
Deployments are the final piece of the puzzle before an API goes live. Stages and deployments must be associated together so that a deployment knows what API configuration needs to be deployed. A deployment is considered a resource in AWS, but what it really does is take the current state of an API and publish it. My guess as to why AWS considers the deployment itself a resource is because those deployments are immutable. Immutability in AWS must mean that internally they need to capture and store it as a resource with a certain set of configurations.
...
We event have access to configuration that lets us write a regex for initial validation before sending it to the Lambda Authorizer, so if a malformed token is sent in it is automatically rejected without using Lambda execution time. After the Authorizer Lambda is invoked, it receives relevant request information and must construct and return an IAM policy, which probably sounds more complicated than it is. There are numerous examples online from both AWS and third-parties with proper code to handle IAM policy construction, so do not be deterred from using Authorizers because they involve IAM.

My personal preference when it comes to using Lambda Authorizers is the token type configured to look at the standard `Authorization` header. This pulls out the `Authorization` header for my Lambda and caches the exact token so my Lambda does not need to run every time the token's owner calls my API. I use Bearer Tokens to authenticate users, so I pull the JWTs out of those header values to be verified. Once a user is verified, I pull any additional context in to construct the IAM policy and return it.

Another fun bit about Authorizer Lambdas is that we can return extra context which will subsequently be passed to the Lambdas integrated with our API. I find that any of a user's information that some of my API-integrated Lambdas need is best retrieved in the Authorizer Lambda to then be cached and available in subsequent calls. Since that code and retrieval would have to be run by my API-integrated Lambdas anyway, I might as well run it once in my Authorizer Lambda and then let API Gateway's cache take over for a performance boost.

### API Gateway Service Proxy Integrations

API Gateway and Lambda are a match made in heaven, but API Gateway can integrate with other AWS services directly like DynamoDB or SNS. Options open up for quicker development times and easier maintenance if there is no custom code between the API Gateway and the end service. It is not uncommon to see the trinity of serverless operating together: API Gateway, Lambda, and DynamoDB. Sometimes we don't even need to write code for a Lambda function though. If our code is simply translating the request to a DynamoDB row and inserting that item, then we have a great use case for a direct API Gateway service proxy setup.

All AWS services are exposed through APIs, so we can expose the AWS APIs to our users using service proxy integrations as a thin translation layer between our desired interface and AWS's interface. A service proxy integration in API Gateway can be set up similarly to a Lambda integration except that we configure the API's integration type as an AWS Service Proxy.

Once the API is configured as a service proxy, we can map user input to the other AWS service's API input using special syntax for this purpose, which is Apache's Velocity Template Language. Introducing Velocity in the mix means that there is a slight learning curve here with the mapping template syntax.

It is worth pointing out that a service proxy integration without server-side code (i.e. a Lambda in between) means that we lose the ability to validate and scrub input. This is one of the trade-offs that comes with the simplicity of an integration like this. Nevertheless, I have still had success implementing these types of integrations, and I like to use them when data only needs to be transported instead of transformed. Transformation likely means a Lambda is necessary, while simple transportation does not need a Lambda to facilitate it.

My personal preference regarding API Gateway Service Proxy Integrations is to use them sparingly and for internal use. The largest downside for me is the lack of input validation, so I prefer to present a user with an API that allows me to validate input, then use a proxy integration for non-user-facing-services. This makes sure that I will have had the chance to validate and scrub input by the time it gets to an internal service that works as a service proxy integration. The largest benefit to the service proxy integrations is that they allow us to provide a thin translation layer of sorts instead of dealing with AWS's APIs, which can sometimes be confusing especially to developers who are not familiar with them.
...
The pay-by-use model is what makes serverless extremely attractive to me and a multitude of other developers and teams. A traditional setup would most likely charge me by instance because one API means one VM and I would be charged per VM since it would need to be up and running in a data center at all times, which costs money. AWS and its serverless offerings make much better use of their overall computing power, which is passed down to us in the form of more efficient billing models.

I mentioned that with API Gateway we only pay by *valid* requests. This is the reason why I brought up some of the authentication features of API Gateway like API keys and Lambda Authorizers. If a user hits an API with invalid keys or a Lambda Authorizer denies access, then that request will not be charged to us. Granted with a Lambda Authorizer, we would still pay for the Lambda's execution time. However, I see this as a great benefit to us. I can sleep well at night knowing that if some web crawler or rapid-fire script finds my URL, I will not be charged as long as my API has keys or a Lambda Authorizer configured.
